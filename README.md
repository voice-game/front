...
Voice Game은 사용자의 목소리를 이용해 즐길 수 있는 미니 게임들로 구성된 게임 플랫폼입니다.
...
**장애물 충돌 시 캐릭터 사망 구현**  
캐릭터가 장애물과 충돌하면 목숨이 줄어들며 세 번 충돌시 사망  
사망 시 게임 시작점으로 리셋  
**게임 환경 설정 창**  
  게임 환경에 따라 볼륨 감동 설정  
  게임 난이도 설정  
...
**두 유저 간 Volume 입력 크기를 통해 승패 결정**  
Volume은 누적값으로 계산  

**대기 상태 추가하여 멀티플레이 오작동 방지**  
양쪽 사용자가 모두 입장한 뒤 Manual Modal이 모두 닫혔을 때 대기 상태 해제  
...
1. 방 입/퇴장 관련 통신 (join-room, leave-room)
2. 게임 중 유저 정보 통신 (start-game, player-input)

대부분의 동작은 수월하게 구현했으나, `disconnect`를 조작하는 과정에서 어려움이 있었습니다. 유저가 의도적으로 방을 나간 경우에는 `useEffect`에서 이벤트를 emit하여 핸들링할 수 있으나 새로고침, 인터넷 연결 종료 등의 특정 상황에서는 socket에 기본적으로 내장되어 있는 `disconnect` 이벤트가 자동으로 실행되기 때문에 콜백함수에 인자를 전달하기 어렵기 때문입니다.
...
개발 초기에는 Canavas, AudioContext 등 낯선 기술에 여유가 없었기에, 미니 게임 3종 구현에 우선순위를 두고 개발을 하였습니다. 독립된 3개의 게임이지만 멀티 플레이, 마이크 입력, 음성 기반, 이미지 로딩 등 많은 공통된 부분을 가지고 있기에, 어느 정도 게임이 모습을 갖춘 후 아래와 같이 공통로직을 만들기 위한 작업을 진행하였습니다..
...
또한 팀원들에게 학습하는 방식과 태도 등에서 배울 점을 많이 발견하여 기술 외적으로도 발전할 수 있었습니다.

### **`권민호`**

개인프로젝트가 아닌 팀프로젝트다 보니 더욱 책임감을 가지고 개발하였기에 3주간 기술적으로 성장할 수 있었습니다. 특히 공통로직을 만들면서 재사용성에 대해 더 고민하였던 것 같습니다. 이는 추후 개발을 할 때 도움이 될 것이라 생각합니다. 또한 협업을 할때는 옳고/틀림이 아닌 다름의 관점으로 배려하면서 소통하는 것이 중요하다는 것을 배울 수 있었습니다.
